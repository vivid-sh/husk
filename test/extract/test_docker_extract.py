...
class TestDockerExtract:
    ...
    @classmethod
    def assert_running_as_root(cls) -> None:
        assert os.geteuid() == 0, "Must be running as root"

    @classmethod
    def build_test_image(cls) -> None:
        docker_build_dir = Path(__file__).parent / "data"
        ...
        process = subprocess.run(
            [
                ...
                "build",
                "-f",
                str(docker_build_dir / "Dockerfile"),
                str(docker_build_dir),
                "-t",
                cls.test_image_name,
            ],
    ...
    def extract_test_image(cls) -> None:
        main(["extract", "--docker", cls.test_image_name, "-o", str(cls.dist)])
